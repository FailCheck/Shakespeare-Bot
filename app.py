...
# ---------------------------------------------------------
# 2. 저장된 뇌(shakespeare.pt) 불러오기 함수
# ---------------------------------------------------------
@st.cache_resource # 웹사이트가 새로고침 될 때마다 모델을 다시 로딩하지 않게 함 (속도 향상)
def load_model():
    # 파일이 있는지 확인
    try:
        # 저장된 딕셔너리 불러오기
...
# 3. 웹사이트 화면 구성 (UI)
# ---------------------------------------------------------
st.title("📜 Shakespeare AI Writer (Beta)")
st.caption("아직 공부가 부족해 엉뚱한 말을 할 수 있습니다. 귀엽게 봐주세요.")

if model is None:
    st.error("❌ 오류: 'shakespeare.pt' 파일이 없습니다. 13번 코드를 먼저 실행하세요!")
else:
    # 사용자 입력
    user_input = st.text_input("영어 단어를 입력하세요 (예: Shall)", "Shall")

    if st.button("시 작성하기 (Write Poem)"):
        with st.spinner('셰익스피어 빙의 중...'):
